...
class CallProcessor {
    ...
    srf.invite((req, res) => {
      ...
      return offer(optsOffer)
        ...
        .then(({uas, uac}) => {
          ...
          optsOffer['to-tag'] = optsAnswer['to-tag'];
  ...
  async _handleReinvite(dlg, req, res) {
    if (dlg.type === 'uas') {
      try {
        this.logger.info('handling reinvite on uas leg');
        let opts = Object.assign(dlg.rtpengineOpts, {sdp: req.body});
        ...
        opts = Object.assign(dlg.other.rtpengineOpts, {sdp});
      ...
      this.logger.info('handling reinvite on uac leg');
      try {
