...
class CallProcessor {
  ...
  start(srf, rtpEngine, registrar) {
    ...
    srf.invite(async(req, res) => {
      ...
      const rtpEngineOpts = makeRtpEngineOpts(req, ('outbound' === callDirection), ('inbound' === callDirection));
      const optsOffer = {
   ...
   */
   async _handleReinvite(dlg, offer, answer, rtpEngineOpts, req, res) {
    ...
    try {
      const fromTag = dlg.type === 'uas' ? rtpEngineOpts.uas.tag : rtpEngineOpts.uac.tag;
      const toTag = dlg.type === 'uas' ? rtpEngineOpts.uac.tag : rtpEngineOpts.uas.tag;
      let offerMedia = dlg.type === 'uas' ? rtpEngineOpts.uac.mediaOpts : rtpEngineOpts.uas.mediaOpts;
      ...
      if(!req.body) {
        ...
        const {sdp, ack} = await dlg.other.modify(dlg.other.sdp, {noAck:true});
        optsSdp = sdp;
        ackFunc = ack;
      }
      else {
  ...
  async _handleAck(dlg, answer, offer, ack, offerSdp, rtpEngineOpts, req) {
    this.logger.info('Received ACK with late offer: ', req.body);
      ...
      try {
        ...
        const fromTag = dlg.type === 'uas' ? rtpEngineOpts.uac.tag : rtpEngineOpts.uas.tag;
        const toTag = dlg.type === 'uas' ? rtpEngineOpts.uas.tag : rtpEngineOpts.uac.tag;
        let offerMedia = dlg.type === 'uas' ? rtpEngineOpts.uas.mediaOpts : rtpEngineOpts.uac.mediaOpts;
