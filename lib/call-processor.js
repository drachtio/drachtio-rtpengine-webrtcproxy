...
class CallProcessor {
  ...
  start(srf, rtpEngine, registrar) {
    ...
    srf.invite(async(req, res) => {
      ...
      const webrtcFacingProxyCharacteristics = {
        ...
        'DTLS': 'active',
	      'rtcp-mux': ['offer'],
        'flags': ['SDES-no', 'media handover', 'generate mid']
      } ;
  ...
  /**
   * re-INVITE has been received from one side. 
   ...
   async _handleReinvite(dlg, offer, answer, req, res) {
    ...
    try {
      ...
      if(!req.body) {
        ...
        const optsAnswer = Object.assign({}, dlg.other.rtpEngineOpts, 
        dlg.type === 'uac' ? { 'to-tag': dlg.other.sip.localTag } : {}, { sdp });
  ...
  async _handleAck(dlg, answer, offer, ack, offerSdp, req) {
    ...
    try {
      ...
      const optsOffer = Object.assign({}, dlg.other.rtpEngineOpts, { sdp: offerSdp });
      let response = await offer(optsOffer);
      if ('ok' !== response.result) {
        res.send(488);
      ...
      const ackOpts = Object.assign({}, dlg.rtpEngineOpts,
        dlg.type === 'uac' ? { 'to-tag': dlg.other.sip.localTag } : {}, { sdp: req.body });
