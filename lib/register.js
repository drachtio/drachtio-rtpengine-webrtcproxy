...
class Register  {
  ...
  start(srf, registrar) {
    srf.register(mw, (req, res) => {
      ...
      this.logger.info(`UAC registering: ${req.protocol}/${req.source_address}:${req.source_port} with uri ${req.uri}`);
      ...
      srf.request({
        ...
        request.on('response', (response) => {
          ...
          if (200 === response.status) {
            ...
            this.logger.info(`register failed with ${response.status}`);
          }
          this._endSession();
        });
      });
