...
class Register  {
  ...
  start(srf, registrar) {
    srf.register(mw, (req, res) => {
      ...
      this.logger.info({instanceId, regId},
        `UAC registering: ${req.protocol}/${req.source_address}:${req.source_port} with uri ${req.uri}`) ;
          ...
          if (200 === response.status) {
            ...
            if (arr) {
              ...
              if ('register' === req.registration.type) {
                ...
                registrar.removeUserFlow(user, {regId, instanceId}) ;
